...
# Allowing symmetric boundaries or arbitrary shape
@cython.boundscheck(False)
@cython.wraparound(False)
def ddm_flexbound_simulate(float v = 0,
    ...
    # Loop over samples
    for n in range(n_samples):
        y = (-1) * boundaries_view[0] + (w * 2 * (boundaries_view[0]))  # reset starting position 
        t = 0.0 # reset time
        ix = 0 # reset bonudary index
...
def race_model(v = np.array([0, 0, 0], dtype=DTYPE), # np.array expected, one column of floats
    ...
    # Loop over samples
    for n in range(n_samples):
        particles = np.repeat(w * boundaries_view[0], n_particles) # Reset particle starting points
    ...
    return (rts, choices, {**v_dict,
                           # 'a': a, Not sure what a is supposed to be
                           **w_dict,
                           's': s,
                           **boundary_params,
                           'delta_t': delta_t,
...
# ALEX-NOTE We just have to keep the flexbound case and treat constant boundary as a special case
# Simulate (rt, choice) tuples from: Ornstein-Uhlenbeck -------------------------------------------
# def ornstein_uhlenbeck(float v = 1, # drift parameter
...
def ornstein_uhlenbeck_flexbound(float v = 0, # drift parameter
    ...
    return (rts, choices, {'v': v,
                           # 'a': a, a not defined for ornstein_uhlenbeck
                           'w': w,
                           'g': g,
                           's': s,
                           **boundary_params,
...
def lca(v = np.array([0, 0, 0], dtype=DTYPE), # drift parameters (np.array expect: one column of floats)
    ...
    return (rts, choices, {**v_dict,
                           ...
                           **w_dict,
                           ...
                           'b': b,
                           's': s,
                           # **boundary_params, no boundary function for LCA?
                           'delta_t': delta_t,
                           'max_t': max_t,
                           'n_samples': n_samples,
                           # 'simulator': 'ornstein_uhlenbeck',
                           'simulator' : 'lca',
                           # 'boundary_fun_type': boundary_fun.__name__, no boundary function for LCA?
                           'possible_choices': list(np.arange(0, n_particles, 1))})
